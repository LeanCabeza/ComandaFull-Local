...
class PedidoController extends Pedido implements IApiUsable
{
    ...
    public function SacarFotoCliente($request, $response, $args)
    {
      $parametros = $request->getParsedBody()["body"];
      $id_cliente = $parametros['id_cliente'];
      ...
      if (isset($id_cliente) && isset($id_pedido))
      {
        if ($archivo["foto"]) {
          $destino = "./FotosClientes/";
          $extension = explode(".",$archivo["foto"]->getClientFileName());
          $extension = array_reverse($extension)[0];
          $urlFoto = "IdCliente_".$id_cliente."_IdPedido_".$id_pedido."_Fecha_".date('Y-m-d').".". $extension;
          $archivoFoto = $archivo["foto"];
          $archivoFoto->moveTo($destino . $urlFoto);
          $payload = json_encode(array("mensaje" => "Whiskey, foto sacada correctamente!"));
        }else{
          $payload = json_encode(array("mensaje" => "La foto salio borrosa, sacala de nuevo."));
        }
      }else{
        $payload = json_encode(array("mensaje" => "Es necesario que ingreses el id cliente y pedido"));
      }
